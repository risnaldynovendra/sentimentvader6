...
class TweetSentimentApp:
    def __init__(self):
        ...
        self.positive_words = self.read_word_list('positive.txt')
        self.negative_words = self.read_word_list('negative.txt')
        self.constructive_words = self.read_word_list('membangun.txt')
        self.destructive_words = self.read_word_list('menghancurkan.txt')
        self.agitative_words = set(open('agitatif.txt').read().splitlines())
        self.max_sequence_length = 100
        self.multinomial_naive_bayes_model = joblib.load('multinomial_naive_bayes_model.pkl')
        self.tfidf_vectorizer = joblib.load('tfidf_vectorizer.pkl')
        
        # Add Sastrawi components
    ...
    def clean_text(self, text):
        ...
        # Apply Sastrawi for stemming and stopword removal
    ...
    def analyze_sentiment(self, text):
        sentiment_score = self.sent_analyzer.polarity_scores(text)['compound']
    ...
    def read_word_list(self, filename):
        ...
        with open(filename, 'r', encoding='utf-8') as file:
            words = [stopword_remover.remove(line.lower().strip()) for line in file if line.strip()]
    ...
    def analyze_tweets_sentiment(self, dataframe):
        ...
        for index, row in dataframe.iterrows():
            ...
            # Menggunakan stemmer untuk mendapatkan kata dasar
            stemmed_words = [stemmer.stem(word) for word in tweet.split()]
    ...
    def plot_word_frequency_by_user(self, df):
        ...
        for user, sentiment_freq in user_word_frequency.items():
            self.plot_word_frequency(user, sentiment_freq)
        
    def plot_word_frequency(self, user, sentiment_freq):
        plt.figure(figsize=(10, 6))
        
        for sentiment, word_freq in sentiment_freq.items():
            word_freq_df = pd.DataFrame(word_freq.items(), columns=['Word', 'Frequency'])
            plt.bar(word_freq_df['Word'], word_freq_df['Frequency'], label=f'{sentiment.title()}')

        plt.xlabel('Word')
        plt.ylabel('Frequency')
        plt.title(f'Word Frequency for User: {user}')
        plt.xticks(rotation=45)
        plt.legend()
        st.pyplot()
    # =============================================================================================
...
# =============================================================================================
    def detect_agitation(self, tweet):
        ...
        for word in self.agitative_words:
            if re.search(r'\b' + word + r'\b', cleaned_tweet, re.IGNORECASE):
                return True
        return False
    ...
    def clean_text(self, text):
        ...
        cleaned_text = self.stopword_remover.remove(text)
    ...
    def most_constructive_and_destructive(self, dataframe, constructive_words, destructive_words):
        ...
        for index, row in dataframe.iterrows():
            ...
            # Clean the tweet text using Sastrawi
            cleaned_tweet = self.clean_text(tweet)

            constructive_count = sum(1 for word in cleaned_tweet.split() if word in constructive_words)
            destructive_count = sum(1 for word in cleaned_tweet.split() if word in destructive_words)
    ...
    def get_words_by_sentiment(self, dataframe, username, sentiment):
        # Combine all cleaned_text for a specific user and sentiment
    ...
    def plot_wordcloud(self, username, sentiment, words):
        # Clean the combined words using Sastrawi
        cleaned_words = self.clean_text(words)

        wordcloud = WordCloud(width=800, height=400, background_color='black').generate(cleaned_words)
